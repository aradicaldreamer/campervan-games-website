...
`other`, `level`, `email` (optional), plus context: `version`, `platform`, `device` (user agent
and screen), `page` (the full URL the form was opened with), `submittedAt`. A hidden `website`
field is a honeypot - anything that fills it is dropped client-side.
